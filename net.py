...
def main():
  ...
  test_out = np.concatenate((np.expand_dims(np.array(range(X_test.shape[0]), dtype=int), axis=1), np.expand_dims(y_pred_test, axis=1)), axis=1)
...
class LinearLayer:
  ...
  def backward(self, grad):
    ...
    self.grad_weights = self.input.T @ grad # Step 1
...
class FeedForwardNeuralNetwork:
  ...
  def __init__(self, input_dim, output_dim, hidden_dim, num_layers, activation=activation):
